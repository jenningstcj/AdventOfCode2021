...
const fuelSpent = input
  .split(",")
  .map((x) => parseInt(x, 10))
  .sort(Ascending)
  .reduce((acc, value, index, originalArray) => {
    const median = findMedian(originalArray);
    return acc + makePositive(value - median);
  }, 0);

//@ts-ignore
console.log(`PART ONE: ${fuelSpent}`);
...
range(minPos, maxPos).forEach((i) => {
  let total = 0;
  range(0, numPositions).forEach((j) => {
...
//@ts-ignore
console.log(`PART TWO: ${minUsed}`);

//run with:  npx ts-node ./Days/07/07.ts
